...
public class AlgoMinMax9x9 extends AlgoRecherche {
    ...
    public Coup meilleurCoup(Plateau _plateau, Joueur _joueur, boolean _ponder) {
        ...
        for (Coup c : listeCoups) {
            index = 0;
            int noteCoup = minMax(pl, listeCoups, (CoupTicTacToe) c, profondeur, _joueur, true);
            c.setNote(noteCoup);

            System.out.println("Coup principal : " + c + " " + noteCoup + " " + index);
    ...
    private int minMax(Plateau pl, ArrayList<Coup> listeCoupsOpti, CoupTicTacToe node, int profondeur, Joueur joueur, boolean isMaxPlayer) {
        ...
        System.out.println("Liste coups opti : " + listeCoupsOpti);
        
        if (!pl.partieTerminee()) {
        ...
        if(pl.partieGagnee()){
            if(((CoupTicTacToe) pl.getDernierCoup()).getJeton().getJoueur() == joueur){
                return +10;
            }
            else{
                return -10;
    ...
    /*
     private int max(ArrayList<Coup> listeCoupsOpti){
     int max = listeCoupsOpti.get(0).getNote();
     int note;
     ...
     private int min(ArrayList<Coup> listeCoupsOpti){
     int min = listeCoupsOpti.get(0).getNote();
     int note;
